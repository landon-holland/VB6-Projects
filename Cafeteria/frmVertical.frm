...
Const le = 0.8
Const re = 0.2
picGraph.Scale (0, people)-(choices, 0)

For i = 1 To choices
    picGraph.Line (i - re, arrchoices(i))-(i - le, 0), QBColor(i), BF
    lblGraph(i - 1).Visible = True
    lblGraph(i - 1).Left = i - 0.9
    lblGraph(i - 1).Top = arrchoices(i) + 50
    lblGraph(i - 1) = Str(arrchoices(i))
    lblGraph(i - 1).Width = 0.7
